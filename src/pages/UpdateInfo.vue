  ...
  <SanFormList>
    ...
    <div class="san-item">
      ...
      <div class="item-input tel">
        ...
        <cube-button :primary="true" @click="getCode" :disabled="errors.has('Mobile') || Mobile ==='' || isGetCode || Mobile==oldData.Mobile">{{codeText}}</cube-button>
      </div>
      <SanTip :disabled="this.errors.has('Mobile')" :name="Mobile" :msg="errors.first('Mobile')" />
    </div>
    <div class="san-item" v-if="Mobile!=oldData.Mobile">
    ...
    <div class="san-item last">
      <cube-button :primary="true" @click="validateBeforeSubmit">确定修改</cube-button>
    </div>
  </SanFormList>
</template>

<script>
import SanFormList from "../components/common/SanFormList";
import SanTip from "../components/common/SanTip";
import { putCustom, MobileVerifyCode, getCustom } from "../api";
import constant from "../config/constant";
import mix from "../common/mix";
import eventHub from '../plugins/eventHub';
import {getDateArr} from '../plugins/until'
export default {
  name: "UpdateInfo",
  components: {
    SanFormList,
    ...
    validateBeforeSubmit() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          // 提交
          const { Birthday, DiseaseId, FullName, Mobile, Sex } = this.oldData;
          if (
  ...
  beforeMount() {
    getCustom()
      .then(data => {
        ...
        this.oldData = data;
        // const DselectedIndex = [];

        const { Birthday, DiseaseId, FullName, Mobile, Sex } = data;
        this.Birthday = Birthday;
        this.DiseaseId = DiseaseId;
        this.FullName = FullName;
        this.Mobile = Mobile;
        ...
        // 给定默认值
        for (let item of constant.disease) {
          if (item.Id === DiseaseId) {
            this.DiseaseValue = item.Name;
            break;
          }
        }
        ...
        this.datePicker = this.$createDatePicker({
          ...
          min: [1920, 1, 1],
          max: [2020, 12, 31],
          selectedIndex: getDateArr(1920, Birthday),
          onSelect: (selectedVal, selectedIndex, selectedText) => {
            this.Birthday = `${selectedVal[0]}-${selectedVal[1]}-${
              selectedVal[2]
            }`;
          }
        });
        this.picker = this.$createPicker({
          ...
          selectedIndex: [this.DiseaseId - 1],
          onSelect: (selectedVal, selectedIndex, selectedText) => {
            if (selectedVal[0]) {
